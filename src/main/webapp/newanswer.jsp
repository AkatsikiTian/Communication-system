...
<div class="container-fluid">
    <div class="row">
        ...
        <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
            <h2 style="margin: 20px;text-align: center">新的回答信息</h2>
            <div class="table-responsive">
                <table class="table table-striped table-sm">
                    ...
                    <tbody>
                    <c:forEach items="${newanswers}" var="vars" begin="${param.index*5-5}" end="${param.index*5-1}" step="1" varStatus="i">
                        <tr>
                            <td>${vars.title}</td>
                            <td>${vars.time}</td>
                            <td>${vars.cname}</td>
                            <td>${vars.content}</td>
                            <td><button type="button" onclick="javascrtpt:window.location.href='answeraction?id=${vars.id}&sno=${username}&i=${i.index}&methods=detailanswer'" class="btn btn-info" >查看回答</button></td>
                        </tr>
                    </c:forEach>
                    <c:if test="${newanswers=='[]'}"><td colspan="6"><h3 style="text-align: center">当前尚未有新的回答</h3></td></c:if>
                    </tbody>
                </table>
            </div>
            <div class="page">
                <c:if test="${param.index>1}"><a href="newanswer.jsp?index=${param.index-1}&n=${param.n}">上一页</a></c:if>
                <a href="Javascript: void(0)">第${param.index}页  共${param.n}页</a>
                <c:if test="${param.index<param.n}"><a href="newanswer.jsp?index=${param.index+1}&n=${param.n}">下一页</a></c:if>
            </div>
        </main>
    </div>
</div>
