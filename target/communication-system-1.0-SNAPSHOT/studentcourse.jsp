...
<div style="float: right">
    <form action="studentaction" method="post" name="myForm" onsubmit="return validataForm()">
        <input type="text" name="tno" placeholder="输入老师编号进行查询">
        <input type="text" name="name" placeholder="输入学院名称进行查询">
        <input type="hidden" name="methods" value="quary1">
...
<%--            <td>${vars.content}</td>--%>
            ...
            <td><button type="button" class="btn btn-success" onclick="javascrtpt:window.location.href='askquestion.jsp?' +
                    'cno=${vars.cno}&cname=${vars.cname}&name=${vars.name}&content=${vars.content}'+
                    '&tname=${vars.tname}&college=${vars.college}'">提问</button></td>
        </tr>
    </c:forEach>
    <c:if test="${courses=='[]'}"><td colspan="7"><h3 style="text-align: center">无当前无课程信息，您未选修该类课程</h3></td></c:if>
    </tbody>
</table>
<div class="page">
    <c:if test="${param.index>1}"><a href="studentcourse.jsp?index=${param.index-1}&n=${param.n}&sno=${username}">上一页</a></c:if>
    <a href="Javascript: void(0)">第${param.index}页  共${param.n}页</a>
    <c:if test="${param.index<param.n}"><a href="studentcourse.jsp?index=${param.index+1}&n=${param.n}&sno=${username}">下一页</a></c:if>
</div>
</body>
</html>
<script>
